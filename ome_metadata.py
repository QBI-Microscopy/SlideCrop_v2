...
class OMETIFF(OMEBase):
    ...
    def __init__(self, userDetails, inpath, outpath, compression, section_id, total_sections, input_data, pixelregion,
                 outChan, scalefact, rotation):
    ...
    def iter_image(self):
        # assigns the relevant SlideImage object to meta. Don't know why?
        meta = self.meta
        ...
        #scale factors for full resolution to crop resolution size
        ...
        # create pixel object from above values
        pixels = ome.Pixels(
            DimensionOrder=order, ID='Pixels:%s' % (self.instrument),
            SizeX=str(self.sizeX), SizeY=str(self.sizeY), SizeZ=str(self.sizeZ), SizeT=str(self.sizeT),
            SizeC=str(self.sizeC),
        ...
        # if only one channel, similar again
